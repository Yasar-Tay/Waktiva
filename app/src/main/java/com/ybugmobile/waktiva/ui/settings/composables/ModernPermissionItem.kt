...
fun ModernPermissionItem(
    ...
    Row(
        ...
        // Status pill: glass tinted green when granted, red when denied
        val statusColor = if (isGranted) Color(0xFF4CAF50) else Color(0xFFF87171)
        GlassSurface(
            shape = CircleShape,
            tint = statusColor,
            accent = statusColor.copy(alpha = 0.35f)
        ) {
            Text(
